...
size_t Population::nb_voisins_vivants(size_t ci, size_t cj) const {
  ...
  return cpt - (T[ci][cj].estVivante() ? 1 : 0);
}
...
size_t Population::nb_deces()     const { return nb_cellules(Cellule::ROUGE);}
...
Population Population::next() const {
  ...
  for (size_t i = 0 ; i < N ; i++) {
    for (size_t j = 0 ; j < N ; j++) {
      ...
      (POP.T[i][j]).Vivante(voisin ==3 || (voisin==2 && T[i][j].estVivante()));
    }
  }
  return POP;
}
